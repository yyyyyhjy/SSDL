...
class BuildGraph(InMemoryDataset):
    ...
    def process(self):
        ...
        receivers_all = []
        i = 0
        train_file = 'data/HOU/HOU_traj_train.txt'
        test_file = 'data/HOU/HOU_traj_test.txt'
        with open(train_file,'r') as f:
            ...
            df = pd.read_csv('data/gowalla_filter_HOU.txt', sep='\t', header=None,
                             names=['geo_id', 'lat','lng','category'],
                             encoding='ISO-8859-1')

            for poiid in nodes.keys():
                longitude=df[df['geo_id'] == int(poiid)]['lng'].values[0]
                latitude =df[df['geo_id'] == int(poiid)]['lat'].values[0]
